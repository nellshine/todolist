...
| '기본' 카테고리의 존재 방식 | 도메인 정의서 3.2는 "모든 사용자가 암묵적으로 보유"한다고 정의하지만, PRD 7.3은 "회원가입 시 자동 생성 또는 애플리케이션 레벨 암묵 처리 (설계 시 택1)"로 열어두었다. DB 설계 관점에서는 회원가입(FR-1) 트랜잭션 안에서 `categories`에 실제 '기본' 행을 자동 생성해두는 방식을 확정한다(결정 근거: [`docs/decisions/DB-4-default-category-policy.md`](./decisions/DB-4-default-category-policy.md)). 이렇게 하면 `category_id`가 항상 유효한 FK를 가지며, 이관 로직(규칙7)도 별도 분기 없이 일반 카테고리와 동일한 `UPDATE` 문으로 처리할 수 있다. |
...
| 실제 적용된 인덱스 | `backend/src/migrations/001_init.sql`에 의해 `idx_categories_owner_id`, `idx_categories_owner_name`(UNIQUE), `idx_todos_owner_id`, `idx_todos_category_id`, `idx_todos_owner_completed_end_date`가 적용되어 있다. `002_add_todos_start_date_index_candidate.sql.disabled`는 `start_date` 조회 성능을 위한 추가 인덱스 후보이나 확장자가 `.disabled`라 마이그레이션 실행 대상에서 제외된 미적용 상태이며, 실제 적용 여부는 운영 중 쿼리 패턴을 보고 사람이 판단한다. |
